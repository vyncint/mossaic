#!/usr/bin/env bash
# The vendored termlens skill must name the version this crate depends on.
#
...
# wrong signatures, absent APIs, advice that was true one release ago.
#
...
# termlens = { version = "0.10", features = [...] }  ->  0.10
...
if [ "$skill_minor" != "$dep_minor" ]; then
  echo "::error::the vendored termlens skill is written against ${skill_version} but this crate depends on ${dep_version}."
